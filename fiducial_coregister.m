...
% 
% This function employs a method described in
% Mortenson, M. (1985). Geometric Modelling. John Wiley & Sons: New
% York, pp. 353-355.
% 
...
% 
% For use in coregistration of any two sets of cerebral fiducials,
% P & Q are defined by the fiducial points:
% P(1,:) is nasion XYZ
% P(2,:) is right preauricular XYZ
% P(3,:) is left preauricular XYZ
% Similarly for Q in the other set of fiducials.
% The order of these points (nasion, right, left) is essential!
% 
...
% $Revision: 1.2 $ $Date: 2004/11/11 22:49:33 $

% Licence:  GNU GPL, no express or implied warranties
% History:  09/2002, Darren.Weber_at_radiology.ucsf.edu
%           02/2004, Darren.Weber_at_radiology.ucsf.edu
%                    renamed to fiducial_coregister
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


if ~exist('P','var') | ~exist('Q','var'),
  msg = sprintf('FIDUCIAL_COREGISTER: no input P or Q\n');
  error(msg);
elseif isempty(P) | isempty(Q),
  msg = sprintf('FIDUCIAL_COREGISTER: empty input P or Q\n');
  error(msg);
elseif ~(isequal(size(P),size(ones(3,3)))),
  msg = sprintf('FIDUCIAL_COREGISTER: P must be 3x3 matrix\n');
  error(msg);
elseif ~(isequal(size(Q),size(ones(3,3)))),
  msg = sprintf('FIDUCIAL_COREGISTER: Q must be 3x3 matrix\n');
  error(msg);
end
